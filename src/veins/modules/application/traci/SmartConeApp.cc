...
void SmartConeApp::onWSM(WaveShortMessage* wsm) {
    ...
    if (traciVehicle->getRoadId() == mobility->getManager()->getRoadOfInterest()) {
        if (traciVehicle->getLaneId() == mobility->getManager()->getLaneOfInterest()) { // for right lane vehicles, change lane to the left
            traciVehicle->changeLane(1, 1000 * 4000);
            std::cout << "DEBUG: changed lane" << endl;
        }
        traciVehicle->slowDown(suggestedSpeed, 4000);
...
void SmartConeApp::handlePositionUpdate(cObject* obj) {
    ...
    std::string targetRoad = mobility->getManager()->getRoadOfInterest();
    if (!targetRoad.empty() /*&& !sentMessage*/ && traciVehicle->getRoadId() == targetRoad && lastRoad != targetRoad) {
        lastRoad = targetRoad;
        // sentMessage = true;
        // std::cout << "*****msg sent!" << endl;
        WaveShortMessage* wsm = new WaveShortMessage();
        populateWSM(wsm);
        wsm->setWsmData(targetRoad.c_str());
